...
namespace Pipe
{
    class DealReport : IProcess
    {
        ...
        private void CreateInstruPositionFile(string folderPath, Deal[] dealPosition)
        {
            ...
            {
                writer.WriteLine("DealID,TraderID,Quantity,ExecPrice,BookId,TradeDate,Status,InstrumentID,ClearingFee,TransactionFee,Broker,CounterParty,Comment");
                foreach (var deal in dealPosition)
                {
                    writer.WriteLine($"{deal.DealId},{deal.TraderId},{deal.Quantity},{deal.ExecPrice},{deal.BookId},{deal.TradeDate.Value.ToString("G")},{deal.Status},{deal.InstrumentId},{deal.ClearingFee},{deal.TransactionFee},{deal.Broker},{deal.Counterparty},{deal.Comment}");
                }
            }
        }
    }
