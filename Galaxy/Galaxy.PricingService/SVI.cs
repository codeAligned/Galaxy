...
namespace PricingLib
{
    public class SVI:fittageVol
    {
        ...
        public SVI(double[,] data, double forwardPrice)
        {
            _data = data;
        ...
        public double getVol(double strike)
        {
            ...
            return(Math.Sqrt(SVIRawPara[0].Value + SVIRawPara[1].Value * (SVIRawPara[2].Value * (k - SVIRawPara[4].Value) + Math.Sqrt(Math.Pow((k - SVIRawPara[4].Value), 2) + Math.Pow(SVIRawPara[3].Value, 2)))));
        }
