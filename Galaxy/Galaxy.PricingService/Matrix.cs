...
namespace PricingLib
{
    ...
    {
        public SingularMatrixException() : base("Invalid operation on a singular matrix.")
        { }
    }

    public class Matrix
    {
        #region ctorpublic Matrix() {_values = new double[_rowCount, _columnCount];}
        ...
        private LUDecompositionResults LUDecompose()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < _rowCount; i++)
                { pivotArray[i] = i; }//Iterate down the number of rows in the U matrix.
                for (int i = 0; i < _rowCount; i++)
                {
                    //Do pivots first.
