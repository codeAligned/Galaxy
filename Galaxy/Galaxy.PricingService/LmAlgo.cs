...
{
    public static class LmAlgo
    {
        /// <summary>
        /// User-function delegate structure required by MPFit.Solve
        /// </summary>

        /// <param name="p">I - Parameters</param>
        ...
        /// </param>
        /// <param name="vars">I/O - function private data (cast to object type in user function)</param>
        /// <returns></returns>
        public delegate void LmFunc(double[] p, double[] fvec, IList<double>[] dvec, object vars);

        public const string MpfitVersion = "1.1";
        ...
        *
        *     LmFunct funct     - function to be minimized
        ...
        *     LmParams[] pars      - array of npar structures specifying constraints;
        *                          or 0 (null pointer) for unconstrained fitting
        *                          [ see README and mpfit.h for definition & use of LmParams]
        *     LmConfig[] config - pointer to structure which specifies the
        ...
        *     LmResult[] result - pointer to structure, which upon return, contains
        ...
        */
        ...
        /// or 0 (null pointer) for unconstrained fitting
        /// [ see README and LmParams.cs for definition & use of LmParams]
        /// </param>
        ...
        /// </param>
        /// <returns></returns>
        public static int Solve(LmFunc funct, int m, int npar,double[] xall, LmParams[] pars, LmConfig config, object prv,ref LmResult result)
        {
            var conf = new LmConfig();
            int i, j;
            int qanylim = 0;
            ...
            var step = new double[npar];
            var dstep = new double[npar];
            var mpside = new int[npar];
            var ddebug = new int[npar];
            var ddrtol = new double[npar];
            var ddatol = new double[npar];
            ...
            var fvec = new double[m];
            var qtf = new double[nfree];
            var x = new double[nfree];
            var xnew = new double[npar];
            var fjac = new double[m * nfree];

            int ldfjac = m;
            var diag = new double[npar];
            var wa1 = new double[npar];
            var wa2 = new double[npar];
            var wa3 = new double[npar];
            var wa4 = new double[m];
            var ipvt = new int[npar];

            /* Evaluate user function with initial parameter values */
            funct(xall, fvec, null, prv);

            nfev += 1;

            double fnorm = mp_enorm(m, fvec);
        ...
        OUTER_LOOP:
            ...
            mp_fdjac2(funct, m, nfree, ifree, npar, xnew, fvec, fjac, ldfjac,
        ...
        L200:
            ...
            else
            {
                ...
                for (j = 0; j < nfree; j++)
                {
                    ...
                    double ulim1 = ulim[j] * (1 - sgnu * MP_MACHEP0) - ((ulim[j] == 0) ? (MP_MACHEP0) : 0);
                    double llim1 = llim[j] * (1 + sgnl * MP_MACHEP0) + ((llim[j] == 0) ? (MP_MACHEP0) : 0);
            ...
            double temp2 = (Math.Sqrt(alpha * par) * pnorm) / fnorm;
        ...
        L300:
            ...
            {
                MpCovar(nfree, fjac, ldfjac, ipvt, conf.covtol, wa2);
        ...
        private static void mp_fdjac2(LmFunc funct,
                  ...
                  int[] ddebug, double[] ddrtol, double[] ddatol)
        {
            ...
            */
            ...
            double temp = Math.Max(epsfcn, MP_MACHEP0);
            double eps = Math.Sqrt(temp);
            int ij = 0;

            //dvec = (double**)malloc(sizeof(double**) * npar);
            IList<double>[] dvec = new double[npar][];
            ...
            for (j = 0; j < n; j++)
            {
                ...
                if (dside != null && dside[ifree[j]] == 3 && ddebug[ifree[j]] == 0)
                {
                    ...
                    dvec[ifree[j]] = new LmDelimitedArray<double>(fjac, j * m, m); //fjac + j * m;
                  ...
                  double[] rdiag, double[] acnorm, double[] wa)
        {
            ...
            */
            ...
            var aTemp = new LmDelimitedArray<double>(a, ij, n);
            for (j = 0; j < n; j++)
            {
                aTemp.SetOffset(ij);
            ...
            L40:
                ...
                double ajnorm = mp_enorm(m - j, aTemp);
                if (ajnorm == zero)
                {
                    goto L100;
                }
        ...
        private static void MpQrsolv(int n, double[] r, int ldr, int[] ipvt, double[] diag, double[] qtb, double[] x, double[] sdiag, double[] wa)
        {
            /*
            *     **********
            *
        ...
        L150:
            ...
            MpQrsolv(n, r, ldr, ipvt, wa1, qtb, x, sdiag, wa2);
            for (j = 0; j < n; j++)
            {
                wa2[j] = diag[ifree[j]] * x[j];
            }
        ...
        private static double mp_enorm(int n, IList<double> x)
        {
             ...
             */
            double ans, temp;
            const double rdwarf = MP_RDWARF;
            const double rgiant = MP_RGIANT;
            const double one = 1.0;
        ...
        */
        ...
        private static void MpCovar(int n, double[] r, int ldr, int[] ipvt, double tol, double[] wa)
        {
            int k;
            int j0, k0;
            const double one = 1.0;
